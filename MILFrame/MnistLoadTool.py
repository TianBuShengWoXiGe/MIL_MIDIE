...
import numpy as np
import pandas as pd
from MILFrame.FunctionTool import mnist_bag_loader, print_progress_bar


class MnistLoader:

    def __init__(self, po_label=0, bag_size=(10, 50), po_range=(2, 8), bag_num=(100, 100), bag_resize=(30, 30),
                 seed=None, data_type="mnist", data_path=None):
        """
    ...
    def __load_mnist(self, train):
        ...
        for i, (data, label) in enumerate(data_loader):
            ...
            data = data[0][0].numpy()
            data = np.resize(data, self.bag_resize)
            data, label = data.reshape(-1).tolist(), int(label.numpy()[0])
